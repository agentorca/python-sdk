"""
Orca SDK Client for managing cloud sandboxes.
"""
...
from .exceptions import AuthenticationError, OrcaException, SandboxCreationError
from .sandbox import Sandbox


class Client:
    """Main client for interacting with the Orca API."""

    def __init__(self, api_key: str, base_url: str = "https://compute.agentorca.com", timeout: int = 10):
        """
        Initialize the Orca client.

        Args:
            api_key: API key for authentication
            base_url: Base URL of the API (default: https://compute.agentorca.com)
        ...
        self.logger.debug(f"Orca client initialized")
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            raise OrcaException(f"Request timed out ({timeout_str}): {str(e)}")
        except requests.exceptions.ConnectionError as e:
            raise OrcaException(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            raise OrcaException(f"API request failed: {str(e)}")
    ...
    def get_vm_info(self, vm_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            raise OrcaException(f"VM with ID {vm_id} not found")

        except Exception as e:
            raise OrcaException(f"Failed to get VM info: {str(e)}")
