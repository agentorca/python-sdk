#!/usr/bin/env python3
"""
Deployment script for AgentOrca Python package.

This script automates the process of building and deploying the agentorca package to PyPI.
"""
...
def test_import():
    ...
    try:
        ...
        result = subprocess.run([
            sys.executable, "-c",
            "from agentorca import Client; print('✓ Import successful!')"
...
def main():
    parser = argparse.ArgumentParser(description="Deploy agentorca package to PyPI")
    parser.add_argument(
        "--target",
    ...
    print("🚀 AgentOrca Package Deployment Script")
    print("=" * 50)

    version = get_package_info()
    print(f"📋 Package: agentorca v{version}")
    ...
    if success:
        ...
        if args.target == "test":
            print("🔗 Test installation: pip install -i https://test.pypi.org/simple/ agentorca")
        elif args.target == "prod":
            print("🔗 Installation: pip install agentorca")
